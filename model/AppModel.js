...
export default function App() {
  ...
  */
  ...
  const getItems = useCallback(() => {
    let previous = localStorage.getItem('@game:items');
    if (previous == null) {
      return [];
    }

    let content = atob(previous);
    return content.split(',');
  }, []);

  const setItems = useCallback( (newItems) => {
    let currentItems = getItems();
    let itemsStore = currentItems.concat(newItems);
    localStorage.setItem('@game:items', btoa(itemsStore.toString()));

    setItemsRand(itemsStore);
    return true;
  }, [getItems]);
  ...
  useEffect(() => {
    if (indexClick > 0) {
      ...
      console.log(numItemsRand + " === " + indexClick);
